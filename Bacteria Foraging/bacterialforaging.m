function [parameters,optimum_value]=bacterialforaging();
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
p=5;                 % Dimension of the search space

S=8;	% The number of bacteria in the population (for convenience, require this to be an
        % an even number)

Nc=4; % Number of chemotactic steps per bacteria lifetime (between reproduction steps), assumed
        % for convenience to be the same for every bacteria in the population
Ns=2;   % Limits the length of a swim when it is on a gradient
...
Ned=3; % The number of elimination-dispersal events (Nre reproduction steps in between each event)

ped=0.26; % The probabilty that each bacteria will be eliminated/dispersed (assume that 
...
for m=1:S
	P(:,m,1,1,1)=([0.5;0.9;1;100;4].*((2*round(rand(p,1))-1).*rand(p,1))+[0.5;0.9;1;100;4]); 
  end
...
for j=1:Nc
	...
	for i=1:S 
		...
		J(i,j,k,ell)=nutrientsfunc(P(:,i,j,k,ell),flag,iteration); % this is the objective function
%		J(i,j,k,ell)=nutrientsfunc1(P(:,i,j,k,ell),flag);
      ...
      BACTERIA=i;
		...
		J(i,j+1,k,ell)=nutrientsfunc(P(:,i,j+1,k,ell),flag,iteration); %this is the objective function
...
%---------------------------------
	...
	for m=1:S
		if ped>rand  % Generate random number and if ped bigger than it then eliminate/disperse
			P(:,m,1,1,ell+1)=([0.5;0.9;1;100;4].*((2*round(rand(p,1))-1).*rand(p,1))+[0.5;0.9;1;100;4]);
		else
...
P(1,I,J1,K,Ell)=[0.5-(0.1*sin(P(1,I,J1,K,Ell))^2)];
P(2,I,J1,K,Ell)=[0.9+0.1*sin(P(2,I,J1,K,Ell))];
P(3,I,J1,K,Ell)=1+0.1*([sin(P(3,I,J1,K,Ell))]);
P(4,I,J1,K,Ell)=100+[1.5*(sin(P(4,I,J1,K,Ell))).^2];
P(5,I,J1,K,Ell)=4+[0.5*(sin(P(5,I,J1,K,Ell)))];
